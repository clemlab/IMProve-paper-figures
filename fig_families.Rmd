...
p_hmm_acc <- ggplot(p_hmm_acc, aes(x = in_training)) +
    ...
    geom_text(aes(y = .17 - (.032 * as.numeric(cut)), label = pos_count,
...
p_single_auc <- p_single %>%
    do(calc_auc_roc(.)) %>%
    ungroup() %>%
    mutate(y = max(p_single$score) + diff(range(p_single$score)) * .12,
           label = format_cnt(pos_count, count) %>%
               paste(format_auc(auc, lower95, upper95), sep = "\n"),
           x_loc = row_number(family) - .25)

p_single <- ggplot(p_single, aes(x = family, y = score)) +
