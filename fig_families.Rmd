...
nycomps_pfam_best_vector %>%
    ...
    #           data = nycomps_pfam_outcomes) +
    # geom_step(aes(x = auc, y = cumedist, color = conf_grp),
    #           data = nycomps_pfam_vectors) +
    geom_step(aes(x = auc, y = cumedist), linetype = "dotted") +
    geom_point(aes(x = auc, y = cumedist, color = conf_grp, size = count),
               shape = 21) +
    ...
    scale_size(breaks = c(10, 50, 100, 500), range = c(0.25, 3)) +
...
p_nycomps_summary <- nycomps_pfam_all_outcomes %>%
    ...
    xlab("AUC") +
    ylab("Cumulative Density") +
    # scale_size(breaks = c(5, 15, 25), range = c(0.25, 2),
    #            labels = function(x) ifelse(x == 5, "<5",
    #                                        ifelse(x == 25, ">25", x))) +
    scale_x_continuous(limits = c(-.01, 1.01), expand = c(0, 0),
                       labels = function(x) x*100) +
    scale_y_continuous(limits = c(-.01, 1.01), expand = c(0, 0),
...
p_nycomps_best_worst <- p_nycomps_best_worst %>%
    # censor the size interval to make circles easier to read
    # mutate_rows(count > 25, count = 25) %>%
    # mutate_rows(count < 5, count = 5) %>%
    ggplot(aes(x = auc, y = cumedist)) +
