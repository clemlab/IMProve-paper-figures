...
p_hmm_acc <- nycomps_pfam_auc %>%
    ...
    mutate(attr_value = factor(attr_value, c("TRUE", "FALSE", "NONE"))) %>%
    # calculate label positions
    mutate(lab_y = .47) %>%
    mutate_rows(attr_value != "TRUE", lab_y = lab_y - .010) %>%
    mutate_rows(attr_value == "NONE", lab_y = lab_y - .010) %>%
    ggplot(aes(x = grp, color = attr_value)) +
    geom_hline(yintercept = 0.5, linetype = "dashed", color = "lightgrey") +
    geom_pointrange(aes(ymin = upper95, y = auc, ymax = lower95),
                    size = 0.5, fatten = 0.5,
                    position = position_dodge(width = 0.5)) +
    geom_text(aes(y = lab_y, label = fmt_cnt(pos_count, count)),
              size = 5*5/14) +
    ylab("AUC") +
    scale_color_manual(values = brewer.pal(5L, "Dark2"),
                       labels = p_hmm_acc,
    ...
    theme(legend.position = c(.57, .23),
          legend.key.size = unit(8, "pt"),
          legend.title = element_blank(),
          legend.text = element_text(color = brewer.pal(3, "Dark2")[[1L]]),
...
for (idx in 4:11)
    p_hmm_acc$grobs[[15L]]$grobs[[1L]]$grobs[[idx]] <- zeroGrob()

p_hmm_acc$grobs[[15L]]$grobs[[1L]][[1L]][[13L]]$gp$col <-
    brewer.pal(3L, "Dark2")[[2L]]
p_hmm_acc$grobs[[15L]]$grobs[[1L]][[1L]][[14L]]$gp$col <-
...
p_nycomps_summary <- nycomps_pfam_all_outcomes %>%
    ...
    geom_segment(aes(x = min_auc, xend = max_auc, yend = cumedist),
                 alpha = 0.25, size = 0.25) +
    geom_point(aes(color = conf_grp), size = 0.25) +
    geom_text(aes(x = 0.06,
                  y = 0.85 + ifelse(is.na(conf_grp), -.05, conf_grp/10),
...
p_nycomps_best_worst <- p_nycomps_best_worst %>%
    ...
    geom_text(aes(x = .085, y = mean, label = round(diff) %>% paste0("%")),
              size = 5*5/14,
              data = best_worst_segments %>%
...
p_nycomps_selected_families <- nycomps_pfam_outcomes %>%
    ...
    geom_point(aes(x = attr_value, y = score, color = outcome), size = 0.25,
               position = position_quasirandom(dodge.width = .5, cex = 3)) +
    geom_text(
...
```{r}
make_family_plot <- function(df, plot_all = TRUE) {
    data <- df %>%
    ...
    data %<>%
        ...
        mutate(label = paste0(format_cnt(pos_count, count), "\n",
               ...
               xlabel = paste(attr_label, grp, sep = "\n")) %>%
        arrange(attr_value, grp == "All Outcomes", auc)

    to_plot <- nycomps_pfam_outcomes %>%
        inner_join(data)

     ggplot(to_plot, aes(x = paste(attr_label, grp, sep = "\n"), y = score)) +
        geom_point(aes(color = outcome), size = 0.25,
                   position = position_quasirandom(dodge.width = .5)) +
        geom_text(aes(y = max(to_plot$score) + diff(range(to_plot$score)) * .1,
                      label = label),
...
p_single <- nycomps_pfam_auc %>%
    ...
    {make_family_plot(., plot_all = FALSE) +
...
```{r}
p_mfs <- nycomps_pfam_auc %>%
    filter(attr_value == "PF07690.13") %>%
    {make_family_plot(.) +
         theme(axis.title.x = element_blank())} %>%
    ggplotGrob
...
```{r}
p_abc2 <- nycomps_pfam_auc %>%
    filter(attr_value == "PF01061.21") %>%
    make_family_plot %>%
    ggplotGrob
...
f_families <- ggdraw() +
    plot_grid(p_hmm_acc, p_nycomps_summary, p_nycomps_best_worst,
              ...
              align = "none", rel_widths = c(2, 3, 3), nrow = 1L) %>%
    draw_plot(x = 0, y = .70, width = 1, height = .30) +
    plot_grid(p_single, p_mfs, p_abc2,
              labels = letters[4:6], label_size = 8, vjust = 0,
              align = "v", rel_heights = c(2, 3, 3), ncol = 1L) %>%
    draw_plot(x = 0, y = 0, width = 1, height = .70)

f_families %<>% ggplotGrob

# # dont clip d labels
# f_families$grobs[[4L]]$children[[3L]]$children$layout$layout %<>%
#     mutate_rows(name == "panel", clip = "off")
...
if (write_out == TRUE) {
    save_plot(filename = "plots/fig5.pdf",
              ...
              base_height = uconv(136, "mm", "in", "Length"))
}
ggdraw(f_families)
```
