...
nycomps_pfam_outcomes_auc %>%
    select(attr_value, hmm_name, count, pos_count, neg_count,
           auc, lower95, upper95) %>%
    rename(`Pfam ID` = attr_value,
           `Pfam Name` = hmm_name,
           `Expression Trial Count` = count,
           `Positive Outcomes` = pos_count,
           `Negative Outcomes` = neg_count,
           `Lower Bound 95% CI` = lower95,
           `Upper Bound 95% CI` = upper95) %>%
    write_csv("forward-families/Table S4 - Family Performances")
```
