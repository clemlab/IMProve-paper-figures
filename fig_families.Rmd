...
nycomps_pfam_auc %>%
    filter(grp != "All\nGenes", attribute == "hmm_acc", !is.na(attr_value),
           count > 5) %>%
    group_by(attribute, attr_value) %>%
    filter(auc == max(auc)) %>%
    ungroup %>%
    arrange(auc) %>%
    mutate(attr_value = attr_value %>% factor(., ., ordered = TRUE)) %>%
    ggplot() +
    geom_hline(yintercept = 0.5, linetype = "dashed", color = "grey") +
    geom_pointrange(
        aes(x = attr_value, ymin = lower95, y = auc, ymax = upper95)) +
    coord_flip()
...
p_pfam_auc_hist <- ggplot(nycomps_pfam_best_vector) +
    geom_vline(xintercept = 0.5, linetype = "dashed", color = "grey") +
    geom_hline(aes(yintercept = ecdf(auc)(0.5)),
               color = "grey", linetype = "dashed") +
    geom_text(aes(x = 0.25, y = ecdf(auc)(0.5) - 0.05,
                  label = ecdf(auc)(0.5) %>% specify_decimal(2L)),
              size = 5*5/14, color = "#A4A5A5") +
    geom_text(aes(x = 0.75, y = ecdf(auc)(0.5) + 0.05,
                  label = (1 - ecdf(auc)(0.5)) %>% specify_decimal(2L)),
              size = 5*5/14, color = "#A10F1C") +
    stat_ecdf(aes(x = auc)) +
    geom_rug(aes(x = auc, color = auc > 0.5), size = 0.25) +
    xlab("AUC") +
    ylab("Cumulative Density (Pfams)") +
...
p_nycomps_selected_families <- nycomps_pfam_outcomes %>%
    filter(grp != "All\nGenes", attr_value %in% names(pfams_of_interest)) %>%
    semi_join(nycomps_pfam_best_vector, by = c("grp", "attr_value")) %>%
    ggplot() +
    geom_point(aes(x = attr_value, y = score, color = outcome), size = 0.5,
               position = position_beeswarm(dodge.width = .5, cex = 3)) +
    geom_text(aes(x = attr_value, y = 2,
                  label = paste(format_auc(auc, lower95, upper95),
                                format_cnt(pos_count, count), sep = "\n")),
              size = 5*5/14,
              data = nycomps_pfam_best_vector %>%
                  filter(attr_value %in% names(pfams_of_interest))) +
    geom_text(aes(x = attr_value, y = 3, label = gsub("\n", "", grp)), size = 5*5/14,
              data = nycomps_pfam_best_vector %>%
                  filter(attr_value %in% names(pfams_of_interest))) +
    scale_color_manual(values = c(`TRUE` = "#A10F1C", `FALSE` = "#A4A5A5")) +
    scale_x_discrete(expand = c(0, 0), labels = pfams_of_interest,
