...
# ecoli_xy_boot_out <- boot(data=filter(ecoli_training, cterm == "out") %>% select(ml21, outcome), 
#                statistic=cor_boot, R=bs_count, parallel="multicore", ncpus=25)
...
# ecoli_xy_boot_out_ci <- boot.ci(ecoli_xy_boot_out, type="bca")
...
nycomps_auc <- 
    rbind(nycomps_auc,
          data.frame(ml21 = ci.auc(gene_outcome == "≥1 pos" ~ ml21, 
                    ...
                    pos_count = sum(nycomps_posmix$gene_outcome == "≥1 pos")
                    ),
          data.frame(ml21 = ci.auc(gene_outcome == "≥1 pos" ~ ml21, 
                    ...
                    pos_count = sum(filter(nycomps_posmix, cterm == "CYTOPLASMIC.")$gene_outcome == "≥1 pos")
                    ),
          data.frame(ml21 = ci.auc(gene_outcome == "≥1 pos" ~ ml21, 
                    ...
                    pos_count = sum(filter(nycomps_posmix, cterm == "NON-CYTOPLASMIC.")$gene_outcome == "≥1 pos")
                    ),
          data.frame(ml21 = ci.auc(plasmid_outcome == 1 ~ ml21, 
                    ...
                    pos_count = sum(nycomps_vectors$plasmid_outcome == 1)
                    ),
          data.frame(ml21 = ci.auc(plasmid_outcome == 1 ~ ml21, 
                    ...
                    pos_count = sum(filter(nycomps_vectors, cterm == "CYTOPLASMIC.")$plasmid_outcome == 1)
                    ),
          data.frame(ml21 = ci.auc(plasmid_outcome == 1 ~ ml21, 
                    ...
                    pos_count = sum(filter(nycomps_vectors, cterm == "NON-CYTOPLASMIC.")$plasmid_outcome == 1)
                    )
    )

nycomps_auc <- dcast(nycomps_auc, group + type + cterm + count + pos_count ~ ci_labels, value.var = "ml21")

nycomps_genes <- dplyr::select(nycomps_vectors, orfno, plasmid_name, plasmid_outcome, ml21, cterm) %>% 
    dplyr::group_by(orfno, ml21) %>%
    dplyr::summarize(
        pos_count = sum(plasmid_outcome == 1),
        rel_count = sum(plasmid_outcome)
    ) %>% ungroup %>%
    gather(outcome_key, outcome_value, pos_count:rel_count, factor_key = TRUE)

nycomps_roc_bycount <- foreach(thiskey = levels(nycomps_genes$outcome_key), .combine=rbind) %do% {
    thisdata <- filter(nycomps_genes, outcome_key == thiskey)
    thresholds <- filter(nycomps_genes, outcome_key == thiskey) %>% 
        dplyr::select(outcome_value) %>% 
        arrange(desc(outcome_value)) %>% 
        unique %>% head(-1)
    foreach(thisthreshold = thresholds[[1]], .combine=rbind) %dopar% {
        thisdata <- mutate(thisdata, thisresponse = outcome_value >= thisthreshold)
        df <- data.frame(
            my_roc(thisresponse ~ ml21, data = thisdata, direction = "<"),
            outcome_key = thiskey,
        ...
        # add NA between groups for geom_path with facet_wrap
        df <- rbind(df[1,], df)
        df[1,c("specificities", "sensitivities")] <- NA
        df
    }
}

nycomps_auc_bycount <- foreach(thiskey = levels(nycomps_genes$outcome_key), .combine=rbind) %do% {
    thisdata <- filter(nycomps_genes, outcome_key == thiskey)
    thresholds <- filter(nycomps_genes, outcome_key == thiskey) %>% 
        dplyr::select(outcome_value) %>% 
        arrange(desc(outcome_value)) %>% 
...
nycomps_auc_bycount <- dcast(nycomps_auc_bycount, outcome_key + threshold + pos_count + count ~ ci_labels, value.var = "ml21")
...
nycomps_median_noRNAss <- ddply(nycomps_posmix_noRNAss, .(gene_outcome), summarize,
                        median_ml21 = median(ml21))

nycomps_xscale_noRNAss <- quantile(nycomps_posmix_noRNAss$ml21, probs = c(.0025,.9975), names = FALSE)
nycomps_xscale_noRNAss[[1]] <- nycomps_xscale_noRNAss[[1]]-1
nycomps_xscale_noRNAss[[2]] <- nycomps_xscale_noRNAss[[2]]+1
nycomps_breaks_noRNAss <- seq(-6,6,2)
...
nycomps_posmix_hist_noRNAss <- rbind(
    custom_histogram_data(nycomps_posmix_noRNAss$ml21, filter(nycomps_posmix, gene_outcome != "neg")$ml21, "Expressed"),
    custom_histogram_data(nycomps_posmix_noRNAss$ml21, filter(nycomps_posmix, gene_outcome == "neg")$ml21, "Not Expressed")
    )
nycomps_posmix_hist_noRNAss <- inner_join(nycomps_posmix_hist_noRNAss,
                                  ddply(nycomps_posmix_hist_noRNAss, "mids", summarise, sumcounts = sum(counts)),
                                  by = "mids")
nycomps_posmix_hist_noRNAss$counts_sumcounts <- nycomps_posmix_hist_noRNAss$counts / nycomps_posmix_hist_noRNAss$sumcounts

nycomps_vectors_hist_noRNAss <- rbind(
    custom_histogram_data(nycomps_vectors_noRNAss$ml21, filter(nycomps_vectors_noRNAss, plasmid_outcome == 1)$ml21, "Positive"),
    custom_histogram_data(nycomps_vectors_noRNAss$ml21, filter(nycomps_vectors_noRNAss, plasmid_outcome == -1)$ml21, "Negative")
    )
nycomps_vectors_hist_noRNAss <- inner_join(nycomps_vectors_hist_noRNAss,
                                  ddply(nycomps_vectors_hist_noRNAss, "mids", summarise, sumcounts = sum(counts)),
                                  by = "mids")
nycomps_vectors_hist_noRNAss$counts_sumcounts <- nycomps_vectors_hist_noRNAss$counts / nycomps_vectors_hist_noRNAss$sumcounts

nycomps_bincount_summary_noRNAss <- 
    ddply(rbind(mutate(nycomps_posmix_hist_noRNAss, group = "posmix"), mutate(nycomps_vectors_hist_noRNAss, group = "vectors")),
      .(group), summarize,
      "Number of Bins Retained" = sum(sumcounts > 12),
      "Total Number of Bins" = sum(sumcounts > 0),
      "Percent Retained" = round(sum(sumcounts > 12) / sum(sumcounts > 0), 3) * 100
      ) %>% select(-group)
rownames(nycomps_bincount_summary_noRNAss) <- c("Expression by Gene", "All Expression Trials")
...
nycomps_svmrank_q75 <- filter(nycomps_roc, group == "posmix",
                              abs(threshold_percentiles - 0.75) < 0.0001)$thresholds %>% mean
...
p2_posmix_histogram <- ggplot(nycomps_posmix) + 
    ...
    geom_vline(xintercept = nycomps_svmrank_q75, linetype = "dashed") + 
...
p2_posmix_roc <- ggplot(filter(nycomps_roc, group == "posmix")) +
    ...
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
    annotate(geom = "text", x = 70, y = 19, label = p2_posmix_roc_label$count, size = 6*5/14, parse=TRUE) +
    annotate(geom = "text", x = 70, y = 10, label = p2_posmix_roc_label$ci, size = 6*5/14, parse=TRUE) +
...
p2_raw_roc <- ggplot(filter(nycomps_roc, group == "vectors")) +
    geom_path(aes(x=100-specificities*100, y=sensitivities*100)) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
    annotate(geom = "text", x = 70, y = 19, label = p2_raw_roc_label$count, size = 6*5/14, parse=TRUE) +
    annotate(geom = "text", x = 70, y = 10, label = p2_raw_roc_label$ci, size = 6*5/14, parse=TRUE) +
    annotate("text", x = 33, y = 90, label = "By Expression\nTrial:", size = 7*5/14, fontface = "bold") + 
    scale_y_continuous(expand = c(0, 0)) +
    scale_x_continuous(expand = c(0, 0)) +
    xlab("False Positive Rate") + 
    ylab("True Positive Rate") + 
...
p2_bycount_roc_rel_count <- ggplot(filter(nycomps_roc_bycount, outcome_key == "rel_count")) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", size = 0.25) +
    geom_path(aes(x=100-specificities*100, y = sensitivities*100, color = threshold), size = 0.25) +
    scale_y_continuous(expand = c(0, 0)) +
    scale_x_continuous(expand = c(0, 0)) +
    scale_color_gradient(low="#A4A5A5", high="#A10F1C", na.value="black") +
    xlab("False Positive Rate") + 
    ylab("True Positive Rate") +
    ggtitle("Relative Outcomes (positive-negative)") + 
    scale_fill_manual(values = c("#A4A5A5","#A10F1C")) + 
    theme(aspect.ratio = 1, legend.position = "None", plot.margin=unit(c(0, 0, 0, 0), "in"))

p2_bycount_roc_pos_count <- ggplot(filter(nycomps_roc_bycount, outcome_key == "pos_count")) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", size = 0.25) +
    geom_path(aes(x=100-specificities*100, y = sensitivities*100, color = threshold), size = 0.25) +
    scale_y_continuous(expand = c(0, 0)) +
    scale_x_continuous(expand = c(0, 0)) +
    ...
    xlab("False Positive Rate") + 
    ylab("True Positive Rate") +
    ggtitle("Positive Outcomes") + 
    scale_fill_manual(values = c("#A4A5A5","#A10F1C")) + 
    theme(aspect.ratio = 1, legend.position = "None", plot.margin=unit(c(0, 0, 0, 0), "in"))

p2_bycount_auc <- tableGrob(mutate(nycomps_auc_bycount,
                                   auc = specify_decimal(auc*100, 1),
                                   lower95 = specify_decimal(lower95*100, 1),
                                   upper95 = specify_decimal(upper95*100, 1),
                                   'AUC (95% CI)' = paste0(auc, " (", lower95, "-", upper95, ")")) %>%
                                select(-auc, -lower95, -upper95, -count),
                            theme = table_theme2,
                            rows = NULL)

p2_bycount_auc <- change_cell(p2_bycount_auc, 2, 1, tnew = 2, bnew = 9)
p2_bycount_auc <- change_cell(p2_bycount_auc, 10, 1, tnew = 10, bnew = 23)

p2_bycount_auc <- ggdraw() +
    draw_plot(p2_bycount_auc, x = 0, y = 0, width = .5, height = 1) + 
    draw_plot(p2_bycount_roc_pos_count, x = .5, y = .5, width = .5, height = .5) + 
    draw_plot(p2_bycount_roc_rel_count, x = .5, y = 0, width = .5, height = .5) + 
    draw_plot_label(label = letters[1:3], x = c(0, .5, .5), y = c(1, 1, .5), size = 8)
    
save_plot(filename = "plots/nycomps_auc_by_positive_count.pdf", plot = p2_bycount_auc, base_width = uconv(136, "mm", "in", "Length"))
...
p2_raw_npv <- ggplot(p2_raw_npv_prep) + 
    ...
    annotate("text", x = 20, y = 55, label = "By Expression Trial:", size = 7*5/14, fontface = "bold") + 
    scale_x_continuous(expand=c(0,0), limits=c(0,100)) +
#    scale_y_continuous(expand=c(0,0), limits=c(0,60), breaks=seq(0, 100, 20)) +
    xlab(expression("Percentile SVM"^"rank"*~"score")) + 
    ylab("NPV") +
...
save_plot(filename = "plots/fig2_w_panels_newtable.pdf", plot = p2_main, base_width = uconv(136, "mm", "in", "Length")) #, base_height = 6)
...
celegans_scores <- read.csv("forward-preds/celegans_mp_wormbase.fna.allstats.csv", header = TRUE, as.is = TRUE) %>%
...
# Yeast Predictions
sgd_preds <- read.csv("forward-preds/sgd_orf_coding.mps.fna.allstats.csv", header = TRUE, as.is = TRUE) %>%
    mutate(group = "Eukaryota",
              species = "Saccharomyces cerevisiae",
              uniprotid = title,
              ml21 = read.table("forward-preds/sgd_orf_coding.mps.fna.allstats.ml21")$V1)

pipca_preds <- read.csv("forward-preds/Picpa1_GeneCatalog_CDS_20130227.mps.fna.allstats.csv", header = TRUE, as.is = TRUE) %>%
    mutate(group = "Eukaryota",
...
forward_preds <- rbind(select(euk_preds, group, species, uniprotid, numTMs, ml21), 
                       ...
                       select(sgd_preds, group, species, uniprotid, numTMs, ml21),
                       select(pipca_preds, group, species, uniprotid, numTMs, ml21))
...
```{r}
forward_features <- read.csv("forward-preds/microbial_query.fna.allstats.csv", stringsAsFactors = FALSE, header = TRUE)
microbial_query <- read.delim("forward-preds/microbial_query.dat", header = TRUE)
forward_features$species <- microbial_query$species
rm(microbial_query)

ecoli_features <- read.csv("training/Daley_gfp.fna.allstats.csv", stringsAsFactors = FALSE, header = TRUE)
ecoli_features$species <- "Ec-Daley Training"
ecoli_features <- ecoli_features[, colnames(forward_features)]

nycomps_features <- rbind(read.csv("large-scale/nycomps_only_pos.fna.allstats.csv",
                                   stringsAsFactors = FALSE, header = TRUE),
                          read.csv("large-scale/nycomps_mixed.fna.allstats.csv",
                                   stringsAsFactors = FALSE, header = TRUE),
                          read.csv("large-scale/nycomps_only_neg.fna.allstats.csv",
                                   stringsAsFactors = FALSE, header = TRUE))
nycomps_features$species <- "nycomps all"
nycomps_features <- nycomps_features[, colnames(forward_features)]

nycomps_features$set <- "nycomps"
forward_features$set <- "forward"
ecoli_features$set <- "ecoli"

all_features <- rbind(nycomps_features, ecoli_features, forward_features)

all_features <- filter(all_features, numTMs > 0)
all_features$species <- regmatches(all_features$species, 
                                    regexpr('\\w+\\W{1,2}\\w+', all_features$species, perl=TRUE))

# to recapitulate scaling and centering
load("~/ml-ecoli-paper/predict.RData")
prediction_features <- names(daley_allstats_exp_xtrans_params$mean)

# add missing features, remove unused features
for(x in setdiff(prediction_features, colnames(all_features))){
    all_features[[x]] <- NA
}
rm(x)
all_features <- all_features[, c(prediction_features, "set", "title", "species")]

# Scale and center
#all_features <- predict(daley_allstats_exp_xtrans_params, all_features)

# prepare and plot
all_features <- gather(all_features, featuretype, featurevalue, CAI:GC10max)
all_features$thermophile <- all_features$species %in% c("Aquifex aeolicus", "Spirochaeta thermophila", "Thermus thermophilus")
...
getoverlap_thermo <- function(name) {
    ...
    estimate_overlap(set1, set2)
}

getoverlap_set <- function(name, settype) {
    set1 <- filter(all_features, set == settype, featuretype == name, !is.na(featurevalue))$featurevalue
    set2 <- filter(all_features, set == "ecoli", featuretype == name, !is.na(featurevalue))$featurevalue
    estimate_overlap(set1, set2)
}

getoverlap_species <- function(name, speciesname) {
...
overlap <- data.frame(
    ...
    thermo = mclapply(unique(all_features$featuretype), getoverlap_thermo, 
                      mc.cores = 12, mc.preschedule = FALSE) %>% simplify2array,
    nycomps = mclapply(unique(all_features$featuretype), getoverlap_set, settype = "nycomps", 
                       mc.cores = 12, mc.preschedule = FALSE) %>% simplify2array,
    forward = mclapply(unique(all_features$featuretype), getoverlap_set, settype = "forward", 
                       mc.cores = 12, mc.preschedule = FALSE) %>% simplify2array,
    pf = mclapply(unique(all_features$featuretype), getoverlap_species, speciesname = "Plasmodium falciparum", 
                  mc.cores = 12, mc.preschedule = FALSE) %>% simplify2array,
    ecoli = mclapply(unique(all_features$featuretype), getoverlap_species, speciesname = "Escherichia coli", 
                  mc.cores = 12, mc.preschedule = FALSE) %>% simplify2array,
...
selected_features <- data.frame(
    ...
    x = c(1300, 0.7, 0.4, 100, 0.29, 0.8, 12, 450, .9, 3.5, 480, 0.7),
    y = c(0.0025, 14, 16, 0.018, 28, 5, 0.48, 0.016, 6, 0.5, 0.03, 12), stringsAsFactors = FALSE)
...
selected_density <- foreach(thisfeat=selected_features$feature, .combine = rbind) %dopar% {

    thermo <- filter(all_features, thermophile, featuretype == thisfeat, !is.na(featurevalue))$featurevalue
    pf <- filter(all_features, species == "Plasmodium falciparum", featuretype == thisfeat, !is.na(featurevalue))$featurevalue
    
    training <- filter(all_features, set == "ecoli", featuretype == thisfeat, !is.na(featurevalue))$featurevalue
    range_all <- range(c(thermo, pf, training))
    
    # calculate density at each grid point
#     locfit_thermo <- locfit::locfit(~lp(thermo, adpen = 1.05 * 2*var(thermo), deg=2), 
#                                     ev = lfgrid(mg = 2^13, ll = range_all[[1]], ur = range_all[[2]]),
#                                     maxk = 500, maxit = 500)

#     locfit_thermo <- locfit::locfit(~thermo, alpha = c(0, 0, 1.05 * 2*var(thermo)))
#                                     ev = lfgrid(mg = 2^13, ll = range_all[[1]], ur = range_all[[2]]),
#                                     maxk = 500, maxit = 500)
    
#    locfit_training <- locfit::locfit(~lp(training, nn = 0, h = 0, adpen = 1.05 * 2*var(thermo)), 
#                                    ev = lfgrid(mg = 2^13, ll = range_all[[1]], ur = range_all[[2]]),
#                                    maxk = 500, maxit = 500)
    
    density_thermo <- locfit::density.lf(lp(thermo, nn = 0, h = 0, adpen = 1.05 * 2*var(thermo)), 
                                         n = 2^13, from = range_all[[1]], to = range_all[[2]]) %>% data.frame
        
    density_thermo <- locfit::density.lf(thermo, alpha = c(0, 0, 2*var(thermo)), n = 2^13, 
                                         from = range_all[[1]], to = range_all[[2]]) %>% data.frame
    density_pf <- locfit::density.lf(pf, alpha = c(0, 0, 2*var(pf)), n = 2^13, 
                                         from = range_all[[1]], to = range_all[[2]]) %>% data.frame
    density_training <- locfit::density.lf(training, alpha = c(0, 0, 2*var(training)), n = 2^13, 
                                         from = range_all[[1]], to = range_all[[2]]) %>% data.frame

    # normalize density to 1
    density_thermo$y <- density_thermo$y / sum(density_thermo$y)
    density_pf$y <- density_pf$y / sum(density_pf$y)
    density_training$y <- density_training$y / sum(density_training$y)
    
    density_thermo$group <- "Thermophiles"
    density_pf$group <- "P. falciparum"
    density_training$group <- "E. coli (Training Data)"
    
    # add NA between groups for geom_path with facet_wrap
    density_thermo <- rbind(density_thermo[1,], density_thermo)
    density_pf <- rbind(density_pf[1,], density_pf)
    density_training <- rbind(density_training[1,], density_training)
    density_thermo[1, c("x", "y")] <- c(NA, NA)
    density_pf[1, c("x", "y")] <- c(NA, NA)
    density_training[1, c("x", "y")] <- c(NA, NA)

    df <- rbind(density_thermo, density_pf, density_training)
    df$feature <- thisfeat
    df
}

selected_density$feature <- factor(selected_density$feature, levels = selected_features$feature)
...
selected_density_plot <- mutate(selected_density, featuretype = feature, featurevalue = x) %>% 
    filter(featuretype != 'len1_2loop' | featuretype == 'len1_2loop' & featurevalue < 500) %>%
    filter(featuretype != 'seqLen' | featuretype == 'seqLen' & featurevalue < 3000)


pext2_feature_dist <- ggplot(p2ext_feature_dist_prep) +
     geom_histogram(aes(x=featurevalue, y=..density.., fill=group), alpha=0.7, position = "identity", bins = 50) +
    ...
    geom_text(aes(x = x, y = y*.9, label = specify_decimal(pf, k=2)), size = 5*5/14, color = brewer.pal(3, "Dark2")[[3]],
              data = selected_features) + 
#    geom_line(aes(x=x, y=y, color=group), data = selected_density_plot) + 
    ...
    theme(legend.position="bottom",
          ...
          axis.text.y = element_blank(),
          legend.title = element_blank(),
          legend.margin = unit(c(-.0115,0,0,0), "npc"))

pext2_feature_dist_kde <- ggplot(p2ext_feature_dist_prep) +
    geom_line(aes(x=x, y=y, color=group), data = selected_density_plot) + 
    scale_fill_manual(values = brewer.pal(3, "Dark2")) + 
    scale_color_manual(values = brewer.pal(3, "Dark2")) + 
    scale_y_continuous(expand=c(0,0)) + 
...
mean(abs(overlap$nycomps - overlap$forward), na.rm = TRUE)
mean(abs(overlap$pf - overlap$nycomps), na.rm = TRUE)
mean(abs(overlap$thermo - overlap$nycomps), na.rm = TRUE)
...
p4_main <- ggdraw() + 
    ...
    draw_plot(plot_grid(p4_ecoli_dist, pext2_nycomps_forward, pext2_thermo, pext2_pf, 
                        labels = letters[3:6], label_size = 8, ncol = 1, align = "hv", rel_heights = c(1, 3, 3, 3)),
...
save_plot(filename = "plots/fig4_w_panels_new.pdf", plot = p4_main,
...
# RGB by default
save_plot(filename = "plots/extfig2.pdf", 
          plot = pext2, 
