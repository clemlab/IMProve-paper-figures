...
p1_ecoli_xy <- ggplot(filter(ecoli_training, cterm != "out"), aes(x=ml21, y=outcome, color=cterm)) +
...
#    geom_smooth(aes(color = cterm), span = 100, se = FALSE, fullrange=TRUE, size = 0.5) +
    geom_text(aes(x=x, y=y, color=cterm, label=ci_boot), size = 5*5/14, parse=FALSE, data = ecoli_xy_cor) +
...
#    geom_path(aes(x=thresholds, y=ppv*100), data = filter(nycomps_nvector_ppv_smooth, window == "50")) +
    ...
    scale_color_manual(values = c("#A4A5A5", "#A10F1C")) + 
    scale_fill_manual(values = c("#A4A5A5", "#A10F1C")) + 
    theme_pub(legend.position="None", plot.margin=unit(c(0, 0, 0, 0), "in"))
...
p2_auc_table <- gtable_add_grob(p2_auc_table,
        grobs = segmentsGrob(
            x0 = unit(0,"npc"), y0 = unit(0,"npc"),
            x1 = unit(1,"npc"), y1 = unit(0,"npc"),
            gp = gpar(lwd = 1.0, lty = "dashed")), t = 3, b = 3, l = 2, r = 7)
p2_auc_table <- gtable_add_grob(p2_auc_table,
        grobs = segmentsGrob( 
            x0 = unit(0,"npc"), y0 = unit(0,"npc"),
...
#                 arrow = arrow(angle = 30, length = unit(1.25, "pt"), type = "closed"), data = overlap) +
    ...
    theme_pub(legend.position="none", legend.title = element_blank(), aspect.ratio = 1)
