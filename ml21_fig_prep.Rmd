...
#    geom_path(aes(x=thresholds, y=ppv*100), data = filter(nycomps_nvector_ppv_smooth, window == "50")) +
    geom_hline(aes(yintercept = min(min_ppv)*100), linetype="dashed", color="darkgrey", size = 0.5) +
    annotate("text", x = -1.7, y = 55, label = "Expression in\n(N) His-FLAG-TEV-", size = 7*5/14, fontface = "bold") + 
...
p4_main <- ggdraw() + 
    draw_plot(plot_grid(p4_ecoli, p4_nvector_ppv, nrow = 1, align = "h"), x = -.01, y = 0, width = .72, height = .25) +
    draw_plot(plot_grid(p4_ecoli_dist, pext2_nycomps_forward, pext2_thermo, pext2_pf, 
                        labels = letters[4:7], label_size = 8, ncol = 1, align = "hv", rel_heights = c(1, 3, 3, 3)),
