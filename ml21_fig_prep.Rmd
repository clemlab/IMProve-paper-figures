...
# three NAs from this mapping
nycomps_posmix$orfid <- left_join(nycomps_posmix, nycomps_vectors %>% select(orfno, id) %>% distinct, by = "id")$orfno

# Passed to large_scale
nycomps_posmix$large_scale <- nycomps_posmix$orfid %in% filter(nycomps_allactions, action == "Passed to Large-Scale")$orfno

ci.auc(large_scale ~ ml21, data = nycomps_posmix) %>% str
...
p2_posmix_histogram <- ggplot(nycomps_posmix) + 
    ...
    geom_histogram(aes(x=ml21), fill="blue", binwidth = fd_bw(nycomps_posmix$ml21),
                   data=filter(nycomps_posmix, large_scale == TRUE)) +
...
p2_largescale_auc_ci <- ci.auc(large_scale ~ ml21, data = nycomps_posmix)
p2_largescale_auc_ci <- specify_decimal(p2_largescale_auc_ci*100, k=1)
p2_largescale_auc_ci <- paste0("'", p2_largescale_auc_ci[2],
                               "'~('", p2_largescale_auc_ci[1],
                               "'-'", p2_largescale_auc_ci[3], "')")

    
p2_posmix_roc <- ggplot(filter(nycomps_roc, group == "posmix")) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
    geom_path(aes(x=100-specificities*100, y = sensitivities*100)) +
    geom_path(aes(x=100-specificities*100, y = sensitivities*100), color="blue",
                data=my_roc(large_scale ~ ml21, data = nycomps_posmix) %>% data.frame) +
    ...
    annotate(geom = "text", x = 70, y = 37, color = 'blue',
             label = sum(nycomps_posmix$large_scale), size = 6*5/14, parse=TRUE) +
    annotate(geom = "text", x = 70, y = 28, color = 'blue',
             label = p2_largescale_auc_ci, size = 6*5/14, parse=TRUE) +
