...
ecoli_training <- bind_rows(select(daley_outcomes, ID, cterm, measurement, outcome, ml21, grouping),
...
p1_ecoli_auc <- ggplot(ecoli_auc, aes(x = response_threshold_percentile, color = cterm)) + 
...
p1_ecoli_roc <- ggplot(filter(ecoli_roc, percentile %in% c(50, 97))) + 
    ...
    geom_text(aes(x = .75, y = .8*(.04 - as.numeric(cterm)) - .03, color = cterm, label = ci), 
...
ecoli_xy_cor[ecoli_xy_cor$cterm == "in", c('lower95_boot', 'upper95_boot')] <-
    ecoli_xy_boot_in_ci$bca[c(4,5)]
ecoli_xy_cor[ecoli_xy_cor$cterm == "out", c('lower95_boot', 'upper95_boot')] <-
    ecoli_xy_boot_out_ci$bca[c(4,5)]
ecoli_xy_cor[ecoli_xy_cor$cterm == "fluman", c('lower95_boot', 'upper95_boot')] <-
    ecoli_xy_boot_fluman_ci$bca[c(4,5)]
...
# Figure 2 Data Munging - Prediction in NYCOMPS
...
nycomps_auc <- dcast(nycomps_auc,
                     group + type + cterm + count + pos_count ~ ci_labels,
                     value.var = "ml21")
