...
p2_1 <- ggplot(temp_posmix, aes(x=svmrank, fill=outcome)) + 
    ...
    ylab("Count") +
    xlab(expression( paste( "SVM"^"rank", " score"))) +
    scale_fill_manual(values = c("#00FFFF", "#E08282")) + 
    theme_pub(legend.position="none")
...
p2_2a <- ggplot(subset(temp_hist2, sumcounts > 10)) +
        ...
        scale_y_continuous(expand=c(0,0)) +
        xlab(expression( paste( "SVM"^"rank", " score"))) + ylab("Percentage") +
        scale_fill_manual(values = c("#E08282", "#00FFFF")) + 
        scale_color_manual(values = c("#E08282", "#00FFFF")) +
        theme_pub(legend.position = c(0.1, 0.9), legend.margin = unit(0.6, "cm"))
...
p1 <- ggplot(subset(temp_ppv, window == 50)) + 
...
p3 <- ggplot(subset(temp_ppv, window == 50)) + 
        geom_path(aes(x=c, y=accuracy)) + #, color = window)) +
...
grid.arrange(arrangeGrob(p2_1, p2_2a, p1, ncol = 1, heights=c(2, 1, 1)), 
             arrangeGrob(p2_3, p2_4a, p3, ncol = 1, heights=c(2, 1, 1)), ncol = 2)
...
# mt data munging
pdf("ml21_p3_view_v3.pdf", width = 11, height=8.5)
...
p3_1a <- ggplot(temp_data, aes(x=svmrank, y=assay, color = outcome)) +
    ...
    geom_vline(x = q50, alpha = 0.5, linetype="dashed", color="black") + 
...
temp_data$OutcomeBinary <- NA
temp_data[temp_data$Outcome == "None",]$OutcomeBinary <- "None"
temp_data[temp_data$Outcome != "None",]$OutcomeBinary <- "WB"
...
temp_data2 <- ddply(temp_data, c("OutcomeBinary", "E..coli.strain", "daley_only_raw_exp_ml21"), 
                    summarise, count=length(OutcomeBinary))
p3_2aii <- ggplot() + 
    geom_vline(x = q50, alpha = 0.5, linetype="dashed", color = "black") +
    geom_hline(yintercept = 0, alpha = 0.5, linetype="dashed", color = "darkgrey") +
    geom_bar(aes(x=daley_only_raw_exp_ml21, y=-count), color="#00FFFF", fill="#00FFFF",
             stat="identity", width=.02, data=subset(temp_data2, OutcomeBinary %in% c("None"))) + 
    geom_bar(aes(x=daley_only_raw_exp_ml21, y=count), color="#E08282", fill="#E08282",
             stat="identity", width=.02, data=subset(temp_data2, OutcomeBinary %in% c("WB"))) + 
    xlab(expression( paste( "SVM"^"rank", " score"))) +
    ylab("Count") +
...
p3_2b <- ggplot(subset(archaea_outcomes, Measurement == 'Scanning Densitometry of SDS-PAGE (%)' & 
    ...
    geom_point(aes(x=daley_only_raw_exp_ml21, y=as.numeric(as.character(Outcome))),
                   color="#E08282", fill="#E08282", alpha = 0.7, size = 3) + 
    scale_y_continuous(expand = c(0,0), limits = c(0, 40)) +
    scale_x_continuous(expand = c(0,0), limits = c(-3.2, -1)) +
...
grid.arrange(p3_2b, p3_2aii, ncol=1)
...
p3_3 <- ggplot(temp_data, aes(x=daley_only_raw_exp_ml21, y=outcome, color=outcome)) +
    geom_vline(x = q50, alpha = 0.5, linetype = "dashed", color = "black") +
    geom_jitter(position = position_jitter(height = .05), alpha = 0.7, size = 3) +
