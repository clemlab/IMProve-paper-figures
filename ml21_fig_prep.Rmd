...
filter(ecoli_training, cterm != "out") %>% group_by(cterm) %>%
    summarize(count = length(outcome),
              spearman = cor(outcome, ml21, method = "spearman"),
              lower95 = specify_decimal(spearman_95ci(spearman, count), k=2)[1],
              upper95 = specify_decimal(spearman_95ci(spearman, count), k=2)[2],
...
ecoli_xy_cor <- mutate(ecoli_xy_cor, 
                       ...
                       ci_boot = paste0(spearman, '~(plain(', lower95_boot, ')-plain(', upper95_boot, '))')
                       )
...
p1_ecoli_xy <- ggplot(filter(ecoli_training, cterm != "out"), aes(x = ml21, y = outcome)) +
    ...
    stat_summary(fun.y = mean, geom="point", size = .25, shape = 21) +
...
p1_daley_fluman <- ggplot(ecoli_daley_fluman) + 
    geom_segment(aes(x=fluman_min, xend=fluman_max, y=daley_avg, yend=daley_avg), size=0.25) +
    geom_segment(aes(x=fluman_avg, xend=fluman_avg, y=daley_min, yend=daley_max), size=0.25) +
...
p3_gpcr_1 <- ggplot(p3_gpcr_data) +
    ...
    geom_point(aes(x=x, y=PANEL-1, color=group), shape=124, size=0.95) +
    annotate("text", x = -8.2, y = 1.38, label = "Mammalian GPCRs", size = 7*5/14, fontface = "bold") + 
    xlab(expression( paste( "SVM"^"rank", " score"))) +
    scale_color_gradient(low="#A4A5A5", high="#A10F1C", na.value="black") + 
...
p3_main <- ggdraw() +
    draw_plot(
        plot_grid(p3_archaea_sdspage, p3_archaea_western, p3_mt, p3_gpcr_1,
                  labels = c("a", "","c","e"), label_size = 8, ncol = 1, align = "v"),
