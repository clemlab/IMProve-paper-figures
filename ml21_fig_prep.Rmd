...
header <- tableGrob(data.frame(t(ecoli_stats))[1,1:2], 
                    theme = table_theme,
                    cols=c("Daley, et al., 2005", "Fluman, et al., 2014")) 
...
p1_ecoli_roc <- ggplot(filter(ecoli_roc, percentile %in% c(50, 97))) + 
    ...
    geom_text(aes(x = 80, y = 8*(4-as.numeric(cterm))-3, color = cterm, label = ci), size = 5*5/14,
             data = ecoli_selected_auc) +
...
p1_ecoli_xy <- ggplot(filter(ecoli_training, cterm != "out"), aes(x = ml21, y = outcome)) +t 
    stat_summary(fun.ymin = min, fun.ymax = max, geom = "linerange", size = 0.25) +
    stat_summary(fun.y = mean, geom="point", size = .75, shape = 21) +
    geom_smooth(aes(color = cterm), method=lm, se = FALSE, fullrange=TRUE, size = 0.5)  +
...
p1_main <- plot_grid(p1_table, nrow = 2, ncol = 2) +
    ...
    draw_plot_label(letters[1:4], c(0, 0, 0.5, 0.5), c(1, 0.5, 1, 0.5), size = 8)
...
p2_posmix_roc_label <- paste0(p2_posmix_roc_label$count, "\n", p2_posmix_roc_label$auc, " ", p2_posmix_roc_label$ci)
p2_posmix_roc <- ggplot(filter(nycomps_roc, group == "posmix")) +
    ...
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
    annotate(geom = "text", x = 75, y = 15, label = p2_posmix_roc_label, size = 5*5/14) +
    scale_y_continuous(expand = c(0, 0)) +
    scale_x_continuous(expand = c(0, 0)) +
...
p2_raw_roc_label <- paste0(p2_raw_roc_label$count, "\n", p2_raw_roc_label$auc, " ", p2_raw_roc_label$ci)
p2_raw_roc <- ggplot(filter(nycomps_roc, group == "vectors")) +
    geom_path(aes(x=100-specificities*100, y = sensitivities*100)) +
    geom_ribbon(aes(x=100-specificities*100, ymin=100-specificities*100, ymax=sensitivities*100, fill=sensitivities>1-specificities)) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
    annotate(geom = "text", x = 75, y = 15, label = p2_raw_roc_label, size = 5*5/14) +
    scale_y_continuous(expand = c(0, 0)) +
    scale_x_continuous(expand = c(0, 0)) +
    xlab("False Positive Rate") + 
    ylab("True Positive Rate") + 
...
p2_posmix_ppv <- ggplot(p2_posmix_ppv_prep) + 
    ...
    geom_path(aes(x=threshold_percentiles*100, y=ppv*100), size = 0.25, 
              data = filter(p2_posmix_ppv_prep, type != "min_y") %>% distinct(thresholds, ppv)) +
...
p2_raw_ppv <- ggplot(p2_raw_ppv_prep) + 
    geom_polygon(aes(x=threshold_percentiles*100, y=ppv*100, group=id, fill=y_greater_min, color = y_greater_min), size = 0.1) +
    geom_path(aes(x=threshold_percentiles*100, y=ppv*100), size = 0.25, 
              data = filter(p2_raw_ppv_prep, type != "min_y") %>% distinct(thresholds, ppv)) +
    geom_hline(aes(yintercept = min(min_ppv)*100), linetype="dashed", color="darkgrey", size = 0.5) +
...
#    geom_path(aes(x=thresholds, y=ppv*100), data = filter(nycomps_nvector_ppv_smooth, window == "50")) +
    geom_hline(aes(yintercept = min(min_ppv)*100), linetype="dashed", color="darkgrey", size = 0.5) +
    annotate("text", x = -1.7, y = 55, label = "Expression in 10xHis-FLAG-TEV-", size = 7*5/14, fontface = "bold", family = "Helvetica") + 
...
table_theme2 <-  ttheme_default(
    core = list(fg_params=list(fontsize = 5, fontfamily = "Helvetica")),
    colhead = list(fg_params=list(fontsize = 5, fontfamily = "Helvetica")),
    rowhead = list(fg_params=list(fontsize = 5, parse=TRUE, fontfamily = "Helvetica")),
    padding = unit(c(1, 2), "mm"))

# table_nobg <-  ttheme_minimal(
#     core = list(fg_params=list(fontsize = 5, fontfamily = "Helvetica")),
#     colhead = list(fg_params=list(fontsize = 5, fontfamily = "Helvetica")),
#     rowhead = list(fg_params=list(fontsize = 5, parse=TRUE, fontfamily = "Helvetica")),
#     padding = unit(c(1, 2), "mm"))
...
temp_names <- factor(temp,
               ...
               labels = c("10xHis-FLAG-TEV-", "-TEV-10xHis", "-TEV-10xHis (LDAO)", 
                          "10xHis-GST-TEV", "-TEV-10xHis", "10xHis-TEV-", "10xHis-MBP-TEV", "10xHis-MBP-TEV (LDAO)"))
...
archaea_max_auc <- arrange(archaea_auc, -auc)[1,] %>%
    mutate(auc = specify_decimal(auc*100, k = 1),
           lower95 = specify_decimal(lower95*100, k = 1),
           upper95 = specify_decimal(upper95*100, k = 1),
           ci = paste0(auc, " (", lower95, "-", upper95, ")"))
...
p3_archaea_roc <- ggplot(archaea_roc) + 
    ...
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
    annotate(geom = "text", x = .65, y = .10, label = archaea_max_auc$ci, size = 5*5/14, color = "#AD665D") +
    xlab("False Positive Rate") +
    ylab("True Positive Rate") +
    ggtitle("Archaeal Transporters") + 
...
tcross_max_auc <- filter(tcross_auc, fraction == "MEM", score_type == "ml21") %>% arrange(-auc) 
tcross_max_auc <- tcross_max_auc[1,] %>%
    mutate(auc = specify_decimal(auc*100, k = 1),
           lower95 = specify_decimal(lower95*100, k = 1),
           upper95 = specify_decimal(upper95*100, k = 1),
           ci = paste0(auc, " (", lower95, "-", upper95, ")"))
...
p3_mt_roc <- ggplot(filter(tcross_roc, fraction == "MEM", score_type == "ml21")) + 
    geom_path(aes(x=1-specificities, y=sensitivities, color = factor(positive_threshold))) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
    annotate(geom = "text", x = .65, y = .10, label = tcross_max_auc$ci, size = 5*5/14, color = "#A10F1C") +
...
p3_gpcr_1 <- ggplot(p3_gpcr_data) +
    ...
    geom_point(aes(x=x, y=y, color=group), shape=21, size=0.95) +
    annotate("text", x = -8.2, y = 1.38, label = "Mammalian G-Protein-Coupled Receptors", size = 7*5/14, fontface = "bold") + 
    xlab(expression( paste( "SVM"^"rank", " score"))) +
    scale_color_gradient(low="#A4A5A5", high="#A10F1C", na.value="black") + 
...
lundstrom_max_auc <- filter(lundstrom_auc, host %in% c("E..coli", "P..pastoris")) %>% arrange(-auc)  %>% group_by(host) %>% 
    summarise(positive_threshold = positive_threshold[1],
              auc = specify_decimal(auc[1]*100, k = 1),
              lower95 = specify_decimal(lower95[1]*100, k = 1),
              upper95 = specify_decimal(upper95[1]*100, k = 1),
              ci = paste0(auc, " (", lower95, "-", upper95, ")")) %>% 
    left_join(distinct(p3_gpcr_data, colour, group) %>% mutate(group = as.numeric(group)), by = c("positive_threshold" = "group"))
...
p3_gpcr_roc <- ggplot(group_by(lundstrom_auc, host) %>%
    ...
    geom_path(aes(x=1-specificities, y=sensitivities, color = factor(positive_threshold))) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
    annotate(geom = "text", x = .65, y = .18, label = filter(lundstrom_max_auc, host == "E..coli")$ci, size = 5*5/14, color = "#1B9E77") +
    annotate(geom = "text", x = .65, y = .10, label = filter(lundstrom_max_auc, host == "P..pastoris")$ci, size = 5*5/14, color = "#D95F02") +
    annotate(geom = "point", x = .32, y = .18, size = 1, shape = 15, color = filter(lundstrom_max_auc, host == "E..coli")$colour) +
    annotate(geom = "point", x = .32, y = .10, size = 1,  shape = 15, color = filter(lundstrom_max_auc, host == "P..pastoris")$colour) +
...
tmaritima_max_auc <- arrange(tmaritima_auc, -auc)[1,] %>%
    mutate(auc = specify_decimal(auc*100, k = 1),
           lower95 = specify_decimal(lower95*100, k = 1),
           upper95 = specify_decimal(upper95*100, k = 1),
           ci = paste0(auc, " (", lower95, "-", upper95, ")"))
...
p3_tmaritima_roc <- ggplot(tmaritima_roc) + 
    geom_path(aes(x=1-specificities, y=sensitivities, color = factor(positive_threshold))) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
    annotate(geom = "text", x = .63, y = .10, label = tmaritima_max_auc$ci, size = 5*5/14, color = "#A10F1C") +
...
surade_max_auc <- filter(surade_auc, vector == "pETonly", score_type == "ml21") %>% arrange(-auc)
surade_max_auc <- mutate(surade_max_auc[1,],
                          auc = specify_decimal(auc*100, k = 1),
                          lower95 = specify_decimal(lower95*100, k = 1),
                          upper95 = specify_decimal(upper95*100, k = 1),
                          ci = paste0(auc, " (", lower95, "-", upper95, ")"))
...
p3_surade_roc <- ggplot(filter(surade_roc, vector == "pETonly")) + 
    geom_path(aes(x=1-specificities, y=sensitivities, color = factor(positive_threshold))) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
    annotate(geom = "text", x = .68, y = .10, label = surade_max_auc$ci, size = 5*5/14, color = "#AD665D") +
...
hpylori_max_auc <- filter(hpylori_auc, group == "all", type == "mean") %>% arrange(-auc)
hpylori_max_auc <- mutate(hpylori_max_auc[1,],
                          auc = specify_decimal(auc*100, k = 1),
                          lower95 = specify_decimal(lower95*100, k = 1),
                          upper95 = specify_decimal(upper95*100, k = 1),
                          ci = paste0(auc, " (", lower95, "-", upper95, ")"))
...
#              data = filter(hpylori_auc, group == "all", type == "mean")) +
    annotate(geom = "text", x = .68, y = .10, label = hpylori_max_auc$ci, size = 5*5/14, color = "#A10F1C") +
...
p3_main <- ggdraw() +
    draw_plot(
        plot_grid(p3_archaea_sdspage, p3_archaea_western, p3_mt, p3_gpcr_1,
                  labels = c("a",NULL,"c","e"), label_size = 8, ncol = 1, align = "v"),
        x = 0, y = .23, width = .8, height = 1-.23) + 
    draw_label("Percentage of\nMembrane Protein", x = .03, y = .91, angle = 90, size = 7) + 
    draw_label("Count", x = .03, y = .72, angle = 90, size = 7) + 
    draw_label("Coomassie Blue", x = .22, y = .76, size = 6, colour = "#A10F1C") + 
    draw_label("Western Blot", x = .22, y = .74, size = 6, colour = "#AD665D") + 
    draw_label("No Expression", x = .22, y = .71, size = 6, colour = "#A4A5A5") + 
    draw_plot(
        plot_grid(p3_archaea_roc, p3_mt_roc, p3_gpcr_roc,
                  labels = c("b","d","f"), label_size = 8, ncol = 1, align = "hv"),
        x = .82, y = .17, width = .17, height = 1-.17) +
    draw_plot(
...
p4_ecoli <- ggplot(daley_outcomes) +
    ...
    coord_flip() + 
    ylab(expression(paste("ln"("Whole Cell GFP Fluorescence")))) +
...
p4_main <- ggdraw() + 
    ...
    draw_plot_label(letters[1:3], c(0, 0.5, 0), c(1, 1, 0.7), size = 8)

save_plot(filename = "plots/fig4_w_panels.pdf", plot = p4_main, base_width = uconv(120, "mm", "in", "Length"))

save_plot(filename = "plots/extfig1.pdf", plot = ggdraw(p4_all_violin) + draw_plot_label("a", size = 8), base_width = uconv(120, "mm", "in", "Length"), base_height = 7)
```
