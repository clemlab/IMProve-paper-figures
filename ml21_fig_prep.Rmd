...
colnames(ecoli_stats) <- c("Plates", "Genes", "Observations", "Concordant Pairs", 
                           "Disconcordant Pairs", "Total Pairs", "Kendall's τ")

table_theme <-  ttheme_default(
    core = list(fg_params=list(cex = 0.45)),
    colhead = list(fg_params=list(cex = 0.45)),
    rowhead = list(fg_params=list(cex = 0.45, parse=TRUE)),
...
p1_main <- plot_grid(p1_table, nrow = 2, ncol = 2) +
  ...
  draw_plot(p1_ecoli_roc, 0, 0, 1, .5) +
...
# top of polygon
nycomps_nvector_ppv <- filter(nycomps_roc, group == "N") %>% select(thresholds, ppv) %>% arrange(desc(thresholds))
nycomps_nvector_ppv$ppv_greater_min <- nycomps_nvector_ppv$ppv > filter(nycomps_roc, group == "N")$min_ppv[1]
nycomps_nvector_ppv <- nycomps_nvector_ppv[seq_len(nrow(nycomps_nvector_ppv)) %>% rep(each=2) %>% tail(-1) %>% head(-1),]
rownames(nycomps_nvector_ppv) <- seq(length=nrow(nycomps_nvector_ppv))
nycomps_nvector_ppv$id <-  rep(seq(nrow(nycomps_nvector_ppv)/2), each = 2)
nycomps_nvector_ppv$type <- "ppv"

# bottom of polygon
nycomps_nvector_ppv <- nycomps_nvector_ppv[rep(seq_len(nrow(nycomps_nvector_ppv)), each=2),]
nycomps_nvector_ppv[grepl(".1",rownames(nycomps_nvector_ppv), fixed = TRUE),]$type <- "min_ppv"
nycomps_nvector_ppv[nycomps_nvector_ppv$type == "min_ppv",]$ppv <- filter(nycomps_roc, group == "N")$min_ppv[1]

# order for geom_polygon (needs to be closed)
nycomps_nvector_ppv <- foreach(thisid=unique(nycomps_nvector_ppv$id), .combine = rbind) %do% {
    thisdata <- filter(nycomps_nvector_ppv, id == thisid) %>% 
        arrange(thresholds, ppv)
...
p4_nvector_ppv <- ggplot(nycomps_nvector_ppv) + 
    geom_polygon(aes(x=thresholds, y=ppv*100, group=id, fill=ppv_greater_min, color = ppv_greater_min)) +
...
#    geom_path(aes(x=thresholds, y=ppv*100), data = filter(nycomps_nvector_ppv_smooth, window == "100")) +
    geom_hline(yintercept = filter(nycomps_roc, group == "N")$min_ppv[1]*100, linetype="dashed", color="darkgrey", size = 0.5) +
    coord_cartesian(xlim=nycomps_xscale) +
    scale_x_continuous(expand=c(0,0), breaks=nycomps_breaks) +
    scale_y_continuous(expand=c(0,0), limits=c(0,60), breaks=seq(0, 100, 20)) +
    xlab(expression( paste( "SVM"^"rank", " score"))) + 
...
header <- tableGrob(nycomps_cterm[1, 1:3], 
                    theme = table_theme2,
                    cols = c("All", "C-terminal Cytoplasmic", "C-terminal Periplasmic"))

temp <- factor(temp,
               levels = c("N", "C", "C_LDAO", 
                          "MSGC.24", "MSGC.28", "MSGC.7", "MSGC.9", "MSGC.9_LDAO"),
               labels = c("10xHis-FLAG-TEV-", "-TEV-10xHis", "-TEV-10xHis (LDAO)", 
                          "His-GST-", "-TEV-10xHis", "10xHis-TEV-", "His-MBP-", "His-MBP- (LDAO)"))

p2_auc_table <- tableGrob(nycomps_cterm, theme = table_theme2, 
                 cols = rep(c("Count", "AUC (95% CI)" ), 3),
                 rows = as.character(temp))

p2_auc_table <- gridExtra::join(header[1,], p2_auc_table, along=2) 

# Periplasmic
ind <- find_cell(p2_auc_table, 1, 4, "colhead-fg")
p2_auc_table$layout[ind, c("l", "r")] <- c(6, 7)
ind <- find_cell(p2_auc_table, 1, 4, "colhead-bg")
p2_auc_table$layout[ind, c("l", "r")] <- c(6, 7)

# Cytoplasmic
ind <- find_cell(p2_auc_table, 1, 3, "colhead-fg")
p2_auc_table$layout[ind, c("l", "r")] <- c(4, 5)
ind <- find_cell(p2_auc_table, 1, 3, "colhead-bg")
p2_auc_table$layout[ind, c("l", "r")] <- c(4, 5)

# All
ind <- find_cell(p2_auc_table, 1, 2, "colhead-fg")
p2_auc_table$layout[ind, c("l", "r")] <- c(2, 3)
ind <- find_cell(p2_auc_table, 1, 2, "colhead-bg")
p2_auc_table$layout[ind, c("l", "r")] <- c(2, 3)

# Set widths

p2_auc_table$widths[c(1, 2, 3)] <- unit(c(0.17,.095, 0.175), "npc")
p2_auc_table$widths[c(4, 6)] <- p2_auc_table$widths[2]
p2_auc_table$widths[c(5, 7)] <- p2_auc_table$widths[3]

p2_main <- plot_grid(
    plot_grid(p2_posmix_ppv, p2_raw_ppv,
              p2_posmix_histogram, p2_raw_histogram,
              labels = LETTERS[c(1:4)], ncol = 2, align = "v"),
    plot_grid(p2_posmix_roc, p2_raw_roc, p2_auc_table, 
              labels = LETTERS[c(5:8)], ncol=3, rel_widths = c(1, 1, 2.1)),
...
p3_archaea_sdspage_roc <- ggplot(filter(archaea_roc, positive_threshold == "sds_only")) + 
...
#              data = archaea_auc) +
    xlab("False Positive Rate") +
    ylab("True Positive Rate") +
    scale_color_manual(values = c("#A10F1C"), na.value="black") +
...
p3_archaea_western_roc <- ggplot(filter(archaea_roc, positive_threshold == "wb_sds")) + 
    geom_path(aes(x=1-specificities, y=sensitivities, color = factor(positive_threshold))) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "darkgrey") +
#    geom_text(aes(x=.8, y = 0.2+as.numeric(positive_threshold)/20, color = factor(positive_threshold), 
#                  label=sprintf("AUC=%s (%s-%s)", rnd_pct(auc), rnd_pct(lower95), rnd_pct(upper95))), 
#              data = archaea_auc) +
    xlab("False Positive Rate") +
    ylab("True Positive Rate") +
    scale_color_manual(values = c("#AD665D"), na.value="black") +
    scale_y_continuous(expand = c(0, 0), breaks=c(0, .5, 1), limits=c(0, 1), labels=function(x){x*100}) + 
    scale_x_continuous(expand = c(0, 0), breaks=c(0, .5, 1), limits=c(0, 1), labels=function(x){x*100}) +
    theme_pub(legend.position="None", aspect.ratio = 1,
              axis.title.x = element_blank(), axis.title.y = element_blank(),
...
p3_gpcr_1 <- ggplot(p3_gpcr_data) +
    ...
    ggtitle("Mammalian G Protein-Coupled Receptors") + 
    xlab(expression( paste( "SVM"^"rank", " score"))) +
    scale_color_gradient(low="#A4A5A5", high="#A10F1C", na.value="black") + 
...
p3_main <- plot_grid(p3_archaea_sdspage, p3_archaea_sdspage_roc, 
          p3_archaea_western, p3_archaea_western_roc,
          p3_gpcr_1, p3_gpcr_roc,
          p3_mt, p3_mt_roc,
          plot_grid(p3_hpylori_roc, 
                    p3_surade_roc, 
                    p3_tmaritima_roc, 
                    labels = c("I", "J", "K"), nrow=1), NULL,
          rel_widths = c(3,1), rel_heights = c(1,1,1,1,1),
          labels = letters[c(1:8)], ncol = 2, align = "h")

save_plot(filename = "plots/fig3_w_panels.pdf", plot = p3_main, base_width = uconv(183, "mm", "in", "Length"), base_height = 6)
...
selected_preds <- c("Plasmodium falciparum",
                    "Aquifex aeolicus",
                    "Thermus thermophilus",
                    "Haloarcula marismortui",
                    "Bacillus halodurans",
...
p4_selected_violin <- ggplot(forward_preds) + 
    geom_point(aes(x=species, y=boxplot_3_median), color = "darkgrey", size = 0.5, data = forward_tufte_boxplots) + 
    geom_text(aes(x=species, y=-6, label = species), vjust = 2, fontface = "italic", size = 7*5/14, data = forward_tufte_boxplots) + 
    geom_linerange(aes(x=species, ymin=boxplot_1, ymax=boxplot_2), color = "darkgrey", size = 0.5, data = forward_tufte_boxplots)+ 
    geom_linerange(aes(x=species, ymin=boxplot_4, ymax=boxplot_5), color = "darkgrey", size = 0.5, data = forward_tufte_boxplots) +
    geom_violin2(aes(y=ml21, x=species), color = "black", adjust = 0.3, alpha = 0, weight = 0.5) + 
    # to get the tufte-like boxplots
#    geom_hline(yintercept=forward_ec_thresholds[[3]], linetype="dashed", color="darkgrey") +
    geom_hline(yintercept=forward_ec_thresholds[[4]], linetype="dotted", color="darkgrey", size = 0.5) +
    coord_flip() +
    ylab(expression( paste( "SVM"^"rank", " score"))) +
    scale_y_continuous(expand=c(0,0), limits=nycomps_xscale, breaks=nycomps_breaks) +
    scale_x_discrete(limits=selected_preds) +
    #coord_cartesian(ylim = c(-7,7)) +
...
#    geom_histogram(aes(x=log(gfp)), binwdth = 0.1) +
