...
Sys.setenv(
    JAVA_HOME = '/temp/saladi/jdk1.8.0_25/jre',
    JAVA = '/temp/saladi/jdk1.8.0_25/bin/java',
    JAVAC = '/temp/saladi/jdk1.8.0_25/bin/javac',
    JAVAH = '/temp/saladi/jdk1.8.0_25/bin/javah',
    JAR = '/temp/saladi/jdk1.8.0_25/bin/jar',
    JAVA_LIBS = '-L/temp/saladi/jdk1.8.0_25/jre/lib/amd64/server -ljvm',
    JAVA_CPPFLAGS = '-I/temp/saladi/jdk1.8.0_25/jre/../include -I/temp/saladi/jdk1.8.0_25/jre/../include/linux',
    JAVA_LD_LIBRARY_PATH = '/temp/saladi/jdk1.8.0_25/jre/lib/amd64/server'
    )

Sys.setenv(LD_LIBRARY_PATH = 
    paste(
        Sys.getenv('LD_LIBRARY_PATH'),
        Sys.getenv('JAVA_LD_LIBRARY_PATH'), sep = ':'))
...
p1_ecoli_xy <- ggplot(filter(ecoli_training, cterm != "out"), aes(x = ml21, y = outcome)) +
    stat_summary(fun.ymin = min, fun.ymax = max, geom = "linerange", size = 0.3, alpha=0.35) +
...
p3_trial_counts <- ggplot(group_by(nycomps_vectors, id) %>% 
...
nycomps_cterm <- cbind(filter(nycomps_auc, type == "plasmid" | (type == "all" & group == "vectors"), cterm == "all") %>% 
                           select(-type, -cterm, -pos_count),
                       filter(nycomps_auc, type == "plasmid" | (type == "all" & group == "vectors"), cterm == "CYTOPLASMIC.") %>%
                           select(-group, -type, -cterm, -pos_count),
                       filter(nycomps_auc, type == "plasmid" | (type == "all" & group == "vectors"), cterm == "NON-CYTOPLASMIC.") %>%
                           select(-group, -type, -cterm, -pos_count)
                       )
...
nycomps_cterm <- arrange(nycomps_cterm, desc(all_count))

temp_names <- nycomps_cterm$group

nycomps_cterm <- specify_decimal(select(nycomps_cterm, -group)*100, 1)
...
temp_names <- factor(as.character(temp_names),
...
p2_auc_table <- gtable_add_grob(p2_auc_table,
        grobs = segmentsGrob( 
            ...
            gp = gpar(lwd = 2.0)), t = 1, b = 11, l = 4, r = 4)
p2_auc_table <- gtable_add_grob(p2_auc_table,
            grobs = segmentsGrob(
            x0 = unit(0,"npc"), y0 = unit(0,"npc"),
            x1 = unit(0,"npc"), y1 = unit(1,"npc"),
            gp = gpar(lwd = 2.0)), t = 1, b = 11, l = 6, r = 6)
...
#    annotate(geom="text", x = 0.55, y = 0.92, label = "6325 complete failures (~75%)", size = 6*5/14, color = "#A4A5A5") +
    draw_plot(p3_trial_counts, x = 0, y = .5, width = .33, height = .35) + 
...
save_plot(filename = "plots/fig2_w_panels.pdf", plot = p2_main, base_width = uconv(136, "mm", "in", "Length")) #, base_height = 6)
...
tmaritima_auc <- rbind(
    data.frame(
        ...
        positive_threshold = "Purified",
        pos_count = sum(tmaritima_outcomes$outcome %>% as.numeric >= 3),
        count = length(tmaritima_outcomes$outcome %>% as.numeric >= 3),
        ci_labels = c("lower95", "auc", "upper95")
        ),
    data.frame(
        ci = ci.auc(as.numeric(outcome) >= 2 ~ ml21, direction = "<", data = tmaritima_outcomes),
        positive_threshold = "Expressed",
        pos_count = sum(tmaritima_outcomes$outcome %>% as.numeric >= 2),
        count = length(tmaritima_outcomes$outcome %>% as.numeric >= 2),
        ci_labels = c("lower95", "auc", "upper95")
        )
)

tmaritima_auc <- dcast(tmaritima_auc, positive_threshold + pos_count + count ~ ci_labels, value.var = "ci")
...
tmaritima_auc_counts <- tmaritima_auc$pos_count %>% toString %>% paste
tmaritima_auc_counts <- paste0("list(", tmaritima_auc_counts, "/", tmaritima_auc$count[[1]], ")")
...
p3_tmaritima_roc <- ggplot(tmaritima_roc) + 
    ...
    annotate(geom="text", x=.65, y=.24, label=tmaritima_auc_counts, size = 5*5/14, parse=TRUE) +
...
surade_auc <- dcast(surade_auc, vector + positive_threshold + score_type + pos_count + count ~ ci_labels, value.var = "ci") 
...
surade_auc_counts <- filter(surade_auc, vector == "pETonly", score_type == "ml21")$pos_count %>% rev %>% toString %>% paste
surade_auc_counts <- paste0("list(", surade_auc_counts,"/", filter(surade_auc, vector == "pETonly", score_type == "ml21")$count[[1]], ")")
...
p3_extended <- ggdraw() +
    draw_plot(
        plot_grid(p3_hpylori, p3_surade, p3_tmaritima,
    ...
    draw_plot(
        plot_grid(p3_hpylori_roc, p3_hpylori_auc, p3_surade_roc, p3_tmaritima_roc, rel_heights = c(1, .8, 1, 1),
                  labels = c("b", "", "d", "f"), label_size = 8, ncol = 1, align = "hv"),
...
# get rid of one of the E. coli strains
unique(all_features$species)

all_features <- filter(all_features, numTMs > 0, species != "Escherichia coli (strain ATCC 33849 / DSM 4235 / NCIB 12045 / K12 / DH1)")
...
px_training_forward <- ggplot(filter(all_features, set != "nycomps")) +
    geom_histogram(aes(x=featurevalue, y=..density.., fill=set), alpha=0.5, position = "identity") +
    facet_wrap(~featuretype, ncol=9, scales="free") +
    theme_pub(legend.position="bottom",
          axis.title = element_blank())

px_training_thermophile <- ggplot(filter(all_features, set == "ecoli" | thermophile)) +
    geom_histogram(aes(x=featurevalue, y=..density.., fill=thermophile), alpha=0.5, position = "identity") +
    facet_wrap(~featuretype, ncol=9, scales="free") +
    theme_pub(legend.position="bottom",
          axis.title = element_blank())

px_training_nycomps <- ggplot(filter(all_features, set %in% c("ecoli", "nycomps"))) +
    geom_histogram(aes(x=featurevalue, y=..density.., fill=set), alpha=0.5, position = "identity") +
    facet_wrap(~featuretype, ncol=9, scales="free") +
    theme_pub(legend.position="bottom",
          axis.title = element_blank())

px_nycomps_thermophile <- ggplot(filter(all_features, set == "nycomps" | thermophile)) +
    geom_histogram(aes(x=featurevalue, y=..density.., fill=thermophile), alpha=0.5, position = "identity") +
    facet_wrap(~featuretype, ncol=9, scales="free") +
    theme_pub(legend.position="bottom",
          axis.title = element_blank())

px_training_plasmodium <- ggplot(filter(all_features, set == "ecoli" | species == "Plasmodium falciparum")) +
    geom_histogram(aes(x=featurevalue, y=..density.., fill= set == "ecoli"), alpha=0.5, position = "identity") +
    facet_wrap(~featuretype, ncol=9, scales="free") +
    theme_pub(legend.position="bottom",
          axis.title = element_blank())

px_training_ecoli <- ggplot(filter(all_features, set == "ecoli" | species == "Escherichia coli")) +
    geom_histogram(aes(x=featurevalue, y=..density.., fill= set == "ecoli"), alpha=0.5, position = "identity") +
    facet_wrap(~featuretype, ncol=9, scales="free") +
    theme_pub(legend.position="bottom",
          axis.title = element_blank())

ggsave("plots/training_all_forward.pdf", px_training_forward, width = 22, height = 17)
ggsave("plots/training_thermophile.pdf", px_training_thermophile, width = 22, height = 17)
ggsave("plots/training_nycomps.pdf", px_training_nycomps, width = 22, height = 17)
ggsave("plots/nycomps_thermophile.pdf", px_nycomps_thermophile, width = 22, height = 17)
ggsave("plots/training_plasmodium.pdf", px_training_plasmodium, width = 22, height = 17)
ggsave("plots/training_ecoli.pdf", px_training_ecoli, width = 22, height = 17)

getoverlap <- function(set1, set2) {
    if(length(set1) != 0 & length(set2) != 0) {
        overlapEst(set1, set2, kmax = 10, adjust = c(0.8, NA, NA))[[1]]
    } else {
        NA
    }
}
...
overlap <- data.frame(
    ...
    thermo = mclapply(unique(all_features$featuretype), getoverlap_thermo, mc.cores = 12) %>% simplify2array,
    nycomps = mclapply(unique(all_features$featuretype), getoverlap_set, settype = "nycomps", mc.cores = 12) %>% simplify2array,
    forward = mclapply(unique(all_features$featuretype), getoverlap_set, settype = "forward", mc.cores = 12) %>% simplify2array,
    pf = mclapply(unique(all_features$featuretype), getoverlap_species, speciesname = "Plasmodium falciparum", mc.cores = 12) %>% simplify2array
    )

selected_features <- data.frame(
    names = c("seqLen", "GC", "tAI", "CPS", "relareaSD", "zeroto38avgRNAss",
              "pI", "membrCont", "numPosNormCyt", "delGallTMs", "len1_2loop", "avgRONN"),
    x = c(1300, 0.7, 0.4, 100, 0.29, 0.8, 12, 450, .9, 3.5, 480, 0.7),
    y = c(0.0025, 14, 16, 0.018, 28, 5, 0.48, 0.016, 6, 0.5, 0.03, 12))

selected_features <- gdata:::interleave(selected_features[1:6,], selected_features[7:12,]) %>%
    inner_join(overlap, by = c("names", "feature"))

px_selected <- ggplot(filter(all_features, set == "ecoli" | thermophile, featuretype %in% selected_features) %>%
                          mutate(featuretype = factor(as.character(featuretype), levels = selected_features),
                                 thermophile = factor(thermophile, levels = c(FALSE, TRUE)))) +
    geom_histogram(aes(x=featurevalue, y=..density.., fill=thermophile), alpha=0.5, position = "identity") +
    geom_text(aes(x = 1, y = 1, label = specify_decimal(thermo, k=2)), size = 5*5/14) + 
    scale_fill_manual(values = brewer.pal(3, "Dark2"), labels=c("FALSE" = "E. coli", "TRUE" = "Thermophiles")) + 
    scale_y_continuous(expand=c(0,0)) + 
    scale_x_continuous(expand=c(0,0)) +
...
p_selected_features_labels <- ggplot(filter(overlap, feature %in% selected_features) %>%
                                         mutate(feature = factor(as.character(feature), levels = selected_features))) +
    geom_text(aes(x = 1, y = 1, label = specify_decimal(thermo, k=2)), size = 5*5/14) + 
    facet_wrap(~feature, ncol = 2) + 
    theme_pub(strip.text = element_blank(),
              axis.line = element_blank(),
           #   axis.title = element_blank(),
              axis.text = element_blank(),
              axis.ticks = element_blank())
...
pext2_thermo <- ggplot(select(overlap, -pf) %>% gather(name, value, nycomps:forward), 
                       aes(x=thermo, y=value, label=feature, color=factor(name))) +
    geom_point() + 
    #geom_text(size = 5*5/14, nudge_y = .005) +
    geom_segment(aes(x = thermo, xend = thermo, y = nycomps, yend = forward), color = "black", size = 0.25,
                 arrow = arrow(length = unit(3, "pt"), type = "closed"), data = overlap) +
    xlab("Thermophiles vs. E. coli") +
    ylab("NYCOMPS vs. E. coli − Forward vs. E. coli") +
    scale_x_continuous(limits = c(0,1), expand = c(0,0)) +
    scale_y_continuous(limits = c(0,1), expand = c(0,0)) +
    theme_pub(legend.position=c(.9, .1), aspect.ratio = 1)


pext2_pf <- ggplot(select(overlap, -thermo) %>% gather(name, value, nycomps:forward), 
                       aes(x=pf, y=value, label=feature, color=factor(name))) +
    geom_point() + 
    #geom_text(size = 5*5/14, nudge_y = .005) +
    geom_segment(aes(x = pf, xend = pf, y = nycomps, yend = forward), color = "black", size = 0.25,
                 arrow = arrow(length = unit(3, "pt"), type = "closed"), data = overlap) +
    xlab("P. falciparum vs. E. coli") +
    ylab("NYCOMPS vs. E. coli − Forward vs. E. coli") +
    scale_x_continuous(limits = c(0,1), expand = c(0,0)) +
    scale_y_continuous(limits = c(0,1), expand = c(0,0)) +
    theme_pub(legend.position=c(.9, .1), aspect.ratio = 1)
...
pext2_nycomps_forward <- ggplot(overlap, aes(x=nycomps, y=forward, label=feature)) +
 ...
 #   geom_segment(x=.75, xend=.75, y = 0, yend = .75) + 
    geom_point(size = 0.25) + geom_text(size = 8*5/14, nudge_y = .005) +
    xlab("NYCOMPS vs. E. coli") +
    ylab("Forward vs. E. coli") +
    scale_x_continuous(limits = c(0,1), expand = c(0,0)) +
    scale_y_continuous(limits = c(0,1), expand = c(0,0)) +
    theme(legend.position="bottom", aspect.ratio = 1)

p_feature <- 
    plot_grid(pext2_nycomps_forward, pext2_thermo, pext2_pf, labels = letters[1:3], label_size = 8, ncol = 1, align = "h")


p4_main <- ggdraw() + 
    draw_plot(plot_grid(p4_ecoli, p4_nvector_ppv, nrow = 1, align = "hv"), x = 0, y = 0, width = 1, height = .33) +
    draw_plot(p4_selected_violin, x = 0, y = .33, width = 1, height = .69) +
    draw_plot_label(letters[1:3], c(0, 0, 0.48), c(1, .35, .35), size = 8)
...
save_plot(filename = "plots/extfig2.pdf", 
          plot = plot_grid(px_selected, p4_all_violin, rel_widths = c(1,2), labels = letters[1:2], label_size = 8, ncol = 2), 
