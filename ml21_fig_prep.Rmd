...
nycomps_allactions <- read.csv("large-scale/nycomps-outcomes-all-actions-raw-dump.csv")

nycomps_vectors <- left_join(nycomps_vectors, nycomps_posmix, by = "id") %>%
    filter(!is.na(ml21), plasmid_outcome != 0)

nycomps_vectors$large_scale <- nycomps_vectors$orfno %nin% filter(nycomps_allactions, action == "Passed to Large-Scale")$orfno
...
# large scale split
# ggplot(nycomps_vectors) + geom_histogram(aes(x=ml21, fill=large_scale)) 
