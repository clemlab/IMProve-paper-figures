...
nycomps_vectors_noRNAss <- read.csv("large-scale/nycomps_outcomes.csv", header = TRUE, as.is = TRUE) %>% 
...
p2_posmix_histogram <- ggplot(nycomps_posmix) + 
    ...
    geom_vline(xintercept = nycomps_svmrank_q75$thresholds, linetype = "dashed") + 
...
p2_posmix_roc <- ggplot(filter(nycomps_roc, group == "posmix")) +
    ...
    annotate(geom = "text", x = 70, y = 19, label = p2_posmix_roc_label$count,
             size = 6*5/14, parse=TRUE) +
    annotate(geom = "text", x = 70, y = 10, label = p2_posmix_roc_label$ci,
             size = 6*5/14, parse=TRUE) +
    scale_y_continuous(expand = c(0, 0)) +
    scale_x_continuous(expand = c(0, 0)) +
...
nycomps_auc_feat_subsets_posmix <- foreach(maxidx = seq.int(1, nrow(svmweights)),
        .combine = rbind) %dopar% {
...
ggplot(nycomps_auc_feat_subsets_posmix, aes(thisidx, auc)) +
