...
ecoli_cor <- foreach (thisgrouping = unique(ecoli_training$grouping), .combine='rbind') %dopar% {
    ...
    data.frame(table(pairs), group=thisgrouping)
}
ecoli_auc <- dcast(ecoli_auc, cterm + response_threshold + proportion ~ ci_labels, value.var = "ci")
...
p2_main <- plot_grid(
    plot_grid(p2_posmix_ppv + panel_border(), p2_raw_ppv + panel_border(),
              p2_posmix_histogram + panel_border(), p2_raw_histogram + panel_border(),
              labels = LETTERS[c(1:4)], ncol = 2, align = "v"),
    plot_grid(p2_posmix_roc + panel_border(), p2_raw_roc + panel_border(), p2_auc_table, 
              labels = LETTERS[c(5:8)], ncol=3),
...
p3_archaea_sdspage <- ggplot(filter(archaea_outcomes, Measurement == 'Scanning Densitometry of SDS-PAGE (%)')) +
    ...
    geom_point(aes(x=ml21, y=as.numeric(Outcome)), color="#A10F1C", fill="#A10F1C", size = 3, shape = 95, 
               data = p3_archaea_sdspage_data2) + 
    geom_errorbar(aes(x=ml21, ymin=sds_min, ymax=sds_max), color="#A10F1C", fill="#A10F1C", size = 0.27, width = 0.04,
...
p3_main <- plot_grid(p3_archaea_sdspage + panel_border(), p3_archaea_sdspage_roc, 
          p3_archaea_western + panel_border(), p3_archaea_western_roc,
          p3_gpcr_1 + panel_border(), p3_gpcr_roc,
          p3_mt + panel_border(), p3_mt_roc,
