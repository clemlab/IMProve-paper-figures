...
p_nycomps_cterm <- p_nycomps_cterm %>%
    ...
    geom_text(aes(x = group, y = .378, label = fmt_cnt(pos_count, count)),
              size = 5*5/14,
              data = p_nycomps_cterm %>%
                  filter(cterm == "all")) +
    geom_text(aes(x = group, y = .41 + .032*(cterm == "CYTOPLASMIC."),
    ...
    ylab("AUC") +
    scale_color_manual(
        values = setNames(brewer.pal(5, "Dark2"),
                          c("CYTOPLASMIC.", "NON-CYTOPLASMIC.")),
                       labels = c(`CYTOPLASMIC.` = "Cytoplasmic",
                                  `NON-CYTOPLASMIC.` = "Periplasmic")) +
    labs(color = "C-terminus") +
...
# hide key
for (idx in c(4L, 5L, 7L, 8L)) 
    p_nycomps_cterm$grobs[[8L]]$grobs[[1L]]$grobs[[idx]] <- zeroGrob()

# fix color of periplasmic text
p_nycomps_cterm$grobs[[8L]]$grobs[[1L]]$grobs[[10L]]$gp$col <-
...
p2_main <- ggdraw() +
    ...
    draw_plot(p_nycomps_cterm, x = .27, y = 0, width = .735, height = .38) + 
