...
p2_posmix_ppv_chg <- p2_posmix_ppv_prep %>% 
    ungroup() %>%
    filter(type == "ppv") %>% 
    mutate(percent_imp = (ppv - min_ppv)/min_ppv) %>%
    ...
    geom_point(size = 0.5, data = p2_posmix_ppv_prep_best) +
    xlab(expression("Percentile IMProve score")) +
    ylab("PPV Percent Change") +
    scale_x_continuous(expand = c(0, 0), limits = c(0, 1),
                       labels = function(x) x*100) +
    scale_y_continuous(expand = c(0, 0), # limits = c(0, 2),
                       #breaks = seq(0, 150, .25),
                       labels = function(x) x*100,
                       sec.axis = sec_axis(~ ., labels = function(x) paste0(x, "x"),
                                           name = "PPV Fold Change")) +
...
p2_posmix_ppv_prep %>%
    filter(#outcome == "mix",
           ...
           type == "ppv") %>%
    distinct(outcome, threshold_percentiles, thresholds, ppv, min_ppv) %>%
    arrange(desc(thresholds), outcome) %>%
...
percent_rank_sep <- function(x, all) {
    data <- lapply(x, function(x) myutils:::perc_rank_single(all, x)) %>% unlist
    print(x)
    print(data)
    data
}
...
p2_posmix_histogram <- nycomps_gene_outcomes %>%
    mutate_rows(gene_outcome == "pos", gene_outcome = "mix") %>%
    ggplot(aes(x = score)) +
...
p2_main <- ggdraw() +
    ...
    draw_plot(plot_grid(p2_posmix_ppv, p2_posmix_ppv_chg, nrow = 1, align = 'h',
              ...
              x = 1/3, y = 0.6775, width = 2/3, height = .3225) +
    draw_plot(
