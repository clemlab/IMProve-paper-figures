...
p_indiv_feats <- ggplot(p_indiv_feats, aes(x = old_name, y = auc,
                                           ymin = lower95, ymax = upper95)) +
    geom_hline(yintercept = 0.5, linetype = "dashed") +
    # Have to do this in separate steps to get layering correct
    geom_pointrange(aes(color = outcome), size = 0.5, fatten = 0.5,
                    data = filter(p_indiv_feats, outcome == 1)) +
    geom_pointrange(aes(color = outcome), size = 0.5, fatten = 0.5,
                    data = filter(p_indiv_feats, outcome == 2)) +
    geom_pointrange(aes(color = outcome), size = 0.5, fatten = 0.5,
                    data = filter(p_indiv_feats, outcome == 3)) +
    coord_flip() +
    ylab("AUC") +
...
make_incl_excl <- function(df)
    df %>%
    ggplot(aes(x = category, y = auc,
               ymin = lower95, ymax = upper95)) +
    geom_hline(yintercept = 0.5, linetype = "dashed") +
    # Have to do this in separate steps to get layering correct
    geom_pointrange(aes(color = outcome), size = 0.5, fatten = 0.5,
                    data = filter(df, outcome == 1)) +
    geom_pointrange(aes(color = outcome), size = 0.5, fatten = 0.5,
                    data = filter(df, outcome == 2)) +
    geom_pointrange(aes(color = outcome), size = 0.5, fatten = 0.5,
                    data = filter(df, outcome == 3)) +
    coord_flip() +
    ylab("AUC") +
    scale_x_discrete(limits = names(category_color)) +
    scale_y_continuous(expand = c(0, 0), labels = function(x) x*100) +
...
p_category_incl <- nycomps_all_auc %>%
    filter(feat_type %in% c("all", "cate")) %>%
...
p_category_excl <- nycomps_all_auc %>%
    filter(feat_type %in% c("all", "cate_drop")) %>%
    mutate_rows(feat_type == "all", category = "IMProve Score") %>%
    make_incl_excl() %>%
