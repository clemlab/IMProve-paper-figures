...
feat_subset_auc %>%
 ...
 #   bind_rows(model_auc %>% mutate(iter = "IMProve Score")) %>%
    mutate(iter = factor(iter, feature_order, ordered = TRUE)) %>%
    ggplot() +
    geom_hline(yintercept = 0.5, linetype = "dashed") +
    geom_pointrange(aes(x = "IMProve Score",
...
ecoli_all_auc <- training_data %>%
    ...
    group_stat(svmweights,
               calc_auc_roc, grouping_col = "grp", method = "auc")

ecoli_protnuc_auc <- training_data %>%
    mutate(grp = "ecoli") %>%
    group_stat(svmweights %>% group_by(prot_nuc),
               calc_auc_roc, grouping_col = "grp", method = "auc")

small_all <- small_scale %>%
    group_stat(svmweights, calc_auc_roc, grouping_col = 'dataset')

small_protnuc <- small_scale %>%
    group_stat(svmweights %>% group_by(prot_nuc),
               calc_auc_roc, grouping_col = 'dataset')

small_cate <- small_scale %>%
    group_stat(svmweights %>% group_by(category),
               calc_auc_roc, grouping_col = 'dataset')
```
...
p_ec_auc_base <- ecoli_all_auc %>%
    ...
    scale_y_continuous(expand = c(0L, 0L), breaks = seq(0, 1, .1),
                       limits = c(.03, .97),
                       labels = function(x) x*100) +
    scale_x_continuous(expand = c(0L, 0L), breaks = seq(0L, 1L, .25),
                       limits = c(0, 1),
                       labels = function(x) x*100) +
    ylab("AUC") +
...
```{r small_scale_plots}
p_small_protnuc <- small_protnuc %>%
    bind_rows(small_all %>% mutate(prot_nuc = "IMProve")) %>%
    ungroup %>%
    mutate(dataset = factor(dataset,
                            levels = c("surade", "tmaritima", "hpylori",
                                       "lundstrom_ec", "tcross", "archaea"),
                            labels = c("Microbial\nSecondary\nTransporters", "T. maritima", "H. pylori",
                                       "Mammalian\nGPCRs\n(E. coli)", "M. tuberculosis", "Archaeal\nTransporters")),
