...
```{r summary}
category_order <- svmweights %>%
    group_by(prot_nuc, category) %>%
    summarize(abs_weight = sum(abs(weight))) %>%
    arrange(abs_weight)
...
category_order <- category_order %$%
    unlist(category, use.names = FALSE)
...
p_feat_category_corr <- p_feat_category_corr %>%
    ...
    theme(legend.position = c(-0.2, .73),
...
p_category_incl_excl <- feat_subset_auc %>%
    filter(use_feat_col == "category", order_name %in% c("eq", "ne"),
           dataset != "posmix", dataset != "vectors") %>%
    mutate(iter = factor(iter, unique(feature_order), ordered = TRUE)) %>%
    ggplot() +
    geom_hline(yintercept = 0.5, linetype = "dashed") +
    geom_pointrange(aes(x = "SVMrank Score",
                        y = auc, ymin = lower95, ymax = upper95),
    ...
    coord_flip() +
    theme(legend.position = "none",
          axis.text.y = element_text(color = feature_color %>% rev),
          axis.title.y = element_blank(),
          strip.text = element_blank(),
          strip.background = element_blank(),
          plot.margin = unit(c(0, 0, 0, 0), "pt")) +
    facet_wrap(~order_name, nrow = 1,
               labeller = c(`eq` = "Only Single Category",
                            `ne` = "Excluding Single Category") %>%
                   as_labeller)

p_category_incl_excl <- ggdraw(p_category_incl_excl) +
    draw_plot_label("Only Single\nCategory", x = .12, y = .92,
                    fontface = "bold", size = 6, hjust = 0, vjust = 0) +
    draw_plot_label("Excluding Single\nCategory", x = .59, y = .92,
                    fontface = "bold", size = 6, hjust = 0, vjust = 0)
   
p_category_incl_excl %<>% ggplotGrob
p_category_incl_excl$layout %<>%
    mutate_rows(name == "panel", clip = "off")

ggdraw(p_category_incl_excl)
```
...
p5_weight_importance <-
    ggdraw() + 
    draw_plot(p_weight_man_sum, x = 0, y = .7, width = .67, height = .3) +
    draw_plot(p_feat_category_corr,
              x = .73, y = .71, width = .29, height = .29) +
    draw_plot(p_category_incl_excl, x = 0, y = .21, width = 1, height = .49) +
    draw_plot(
        plot_grid(p_auc_protnuc, p_weight_protnuc, nrow = 1, align = "hv"),
        x = 0, y = 0, width = 1, height = .2) +
    plt_lab(x = 0, y = 1, label = "a") +
    plt_lab(x = .67, y = 1, label = "b") +
...
#if (write_out) {
    save_plot(filename = "plots/fig5_feat2.pdf", plot = p5_weight_importance,
