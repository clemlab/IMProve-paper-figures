...
```{r training_imp}
data <- training_env$ecoli_training %>%
...
ecoli_all <- group_stat(
    group_by(data, cterm), svmweights,
    boot_perm, fn_prefix = "__", cores = 4, count = 5050, force = TRUE)

ecoli_protnuc <- group_stat(
    group_by(data, cterm), group_by(svmweights, prot_nuc),
    boot_perm, fn_prefix = "__", cores = 4, count = 5050, force = TRUE)

ecoli_cate <- group_stat(
    group_by(data, cterm), group_by(svmweights, category),
    boot_perm, fn_prefix = "__", cores = 4, count = 5050, force = TRUE)

small_all <- group_stat(
    small_scale, svmweights,
    calc_auc_roc, grouping_col = 'dataset')

small_protnuc <- group_stat(
    small_scale, group_by(svmweights, prot_nuc),
    calc_auc_roc, grouping_col = 'dataset')

small_cate <- group_stat(
    small_scale, group_by(svmweights, category),
    calc_auc_roc, grouping_col = 'dataset')
```
...
ggplot() +
    ...
    geom_pointrange(aes(x = "IMProve",
                        y = t0, ymin = bca_low, ymax = bca_high,
                        color = cterm, group = cterm),
                    position = position_dodge(width = 0.5),
                    size = 0.5, fatten = 0.5,
                    data = ecoli_all) +
    geom_pointrange(aes(x = prot_nuc,
                        y = t0, ymin = bca_low, ymax = bca_high,
                        color = cterm, group = cterm),
                    position = position_dodge(width = 0.5),
                    size = 0.5, fatten = 0.5,
...
ggplot() +
    ...
    geom_pointrange(aes(x = "IMProve",
                    ...
                    size = 0.5, fatten = 0.5,
                    data = ecoli_all) +
    geom_pointrange(aes(x = category,
                        y = t0, ymin = bca_low, ymax = bca_high,
                        color = cterm, group = cterm),
                    position = position_dodge(width = 0.5),
                    size = 0.5, fatten = 0.5,
